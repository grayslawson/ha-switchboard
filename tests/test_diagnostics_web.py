...
def test_diagnostics_has_safe_filters_and_pagination_contract() -> None:
    ...
    assert 'id="diagnostic-clear"' in page
    assert 'id="diagnostic-status"' in page
    assert "Showing ${first}–${last} of ${diagnosticTotal} diagnostic events." in page
...
def test_diagnostics_render_redacted_summary_fields_without_markup() -> None:
    ...
    assert "No diagnostics have been recorded yet." in page
    assert "const diagnosticFilterIds=" in page
    assert "diagnosticFilterIds.forEach(id=>{el(id).value=''})" in page
