...
| Multi-device | Explicit on/off groups for exposed lights, switches, and fans; whole-home or known area, floor, or label scopes; at most 32 targets; preflight, sequential execution, and partial-result reporting | Home Assistant group membership, atomic batches, toggle batches, parameterized batches, and mixed-domain batches |
...
- Explicit plural on/off requests for exposed lights, switches, or fans can form a bounded group of at most 32 targets. A request may select the whole exposed domain or narrow it by one unambiguous known area, floor, or label. Core preflights every member and executes sequentially; a mid-batch failure can leave earlier verified actions in place. Home Assistant group membership, mixed-domain groups, and other multi-device operations are not yet supported.
