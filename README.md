...
| Multi-device | Explicit on/off groups for exposed lights, switches, and fans; whole-home, known area/floor/label, or explicitly named validated Home Assistant group scopes; at most 32 targets; preflight, sequential execution, and partial-result reporting | Atomic batches, toggle batches, parameterized batches, and groups containing unsupported or unsafe members |
...
- Explicit plural on/off requests for exposed lights, switches, or fans can form a bounded group of at most 32 targets. A request may select the whole exposed domain, narrow it by one unambiguous known area, floor, or label, or name one validated Home Assistant group. Core preflights every member and executes sequentially; a mid-batch failure can leave earlier verified actions in place. Unknown, malformed, unsafe, unavailable, or unsupported group members fail closed. Atomic batches, toggle batches, parameterized batches, and other multi-device operations are not yet supported.
